...
TEST_F(LuaSecurityTest, StandardModeAllowsDestructiveOps) {
  ...
  // io.open for writing
  res = executor_->Execute("run_lua",
                           {{"script",
                             "local f, err = io.open('test_security_std.txt', 'w'); if f then f:write('test'); "
                             "f:close(); return 'true' else return 'err: ' .. tostring(err) end"}});
  ASSERT_TRUE(res.ok()) << res.status().ToString();
...
TEST_F(LuaSecurityTest, MailModeAllowsReadOpen) {
  ...
  auto res = executor_->Execute("run_lua", {{"script",
                                             "local f = io.open('test_read.txt', 'r'); if f then local c = "
                                             "f:read('*a'); f:close(); return c else return 'fail' end"}});
  ASSERT_TRUE(res.ok()) << res.status().ToString();
...
TEST_F(LuaSecurityTest, MailModeBlocksWriteOpenWhenEnvSet) {
  ...
  EXPECT_FALSE(res.ok());
  EXPECT_TRUE(absl::StrContains(res.status().message(), "Mail Model Violation") ||
              absl::StrContains(res.status().message(), "Destructive operations are only allowed"))
