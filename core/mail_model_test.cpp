...
protected:
    void SetUp() override {
        ...
        if (original_branch_.find("### TOOL_RESULT") != std::string::npos) {
             // simplified
        }
...
TEST_F(MailModelTest, Phase1Tools) {
    ...
    auto commit_res = executor_->Execute("git_commit_patch", {
        {"summary", "test: add dummy patch"},
        {"rationale", "Testing the patch commit tool functionality."}
    });
...
TEST_F(MailModelTest, VerifySeries) {
    ...
    auto verify_fail_res = executor_->Execute("git_verify_series", {{"command", "ls next.txt"}, {"base_branch", "HEAD~2"}});
...
TEST_F(MailModelTest, RerollPatch) {
    ...
    // 2. Add patch 1
    {
        std::ofstream ofs("file1.txt"); ofs << "v1"; ofs.close();
    ...
    // 3. Add patch 2
    {
        std::ofstream ofs("file2.txt"); ofs << "v1"; ofs.close();
    ...
    {
        std::ofstream ofs("file1.txt"); ofs << "v2"; ofs.close();
    }
...
TEST_F(MailModelTest, FormatPatchSeries) {
    ...
    // 2. Add patch
    std::ofstream ofs("test.txt"); ofs << "content"; ofs.close();
    auto commit_res = executor_->Execute("git_commit_patch", {{"summary", "test summary"}, {"rationale", "test rationale"}});
