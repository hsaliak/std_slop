...
- [ ] **Task 1.2: Inject Schema Creation:** Add the SQL execution for `CREATE TABLE IF NOT EXISTS lua_functions (name TEXT PRIMARY KEY, code TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP);`.
...
**Goal:** Implement the "Load, Verify, or Delete" injection protocol in C++ when preparing the LCP.
- [ ] **Task 2.1: Locate LCP Preparation:** Identify the C++ logic where `run_lua` prepares the Lua state before executing the agent's script.
- [ ] **Task 2.2: Fetch Functions:** Execute a C++ SQL query to `SELECT name, code FROM lua_functions`.
- [ ] **Task 2.3: Compile & Execute Loop:** Iterate over the result set in C++. For each row, push the code to the Lua state and attempt to compile and evaluate it using `luaL_dostring` (or equivalent `load`/`pcall` C API sequence) to retrieve the function closure.
...
- [ ] **Task 2.5: Active Cruft Cleanup:** If compilation or execution fails, immediately execute a `DELETE FROM lua_functions WHERE name = ?` query from C++ to remove the corrupt function from the database. 

## Phase 3: The `persist_function` API (The Write-Path)
**Goal:** Build a robust, low-boilerplate tool for the LLM to validate and permanently save Lua code.
- [ ] **Task 3.1: Tool Registration:** Define `tools.persist_function` within the Lua Control Plane environment.
...
- [ ] **Task 3.6: DB Storage:** If all checks pass, execute `INSERT OR REPLACE INTO lua_functions (name, code) VALUES (?, ?)`. Return `true, "Function persisted successfully"`.
...
- [ ] **Task 4.3: Concrete Example:** Add a highly visible, copy-pasteable example in `system_prompt.md` demonstrating exactly how to draft, test, and persist a function in a single LCP turn.
