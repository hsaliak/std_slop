...
absl::StatusOr<std::vector<Database::Message>> Database::GetConversationHistory(const std::string& session_id) {
  const char* sql = "SELECT id, session_id, role, content, tool_call_id, status, created_at, group_id FROM messages WHERE session_id = ? AND status != 'dropped' ORDER BY created_at ASC";
  sqlite3_stmt* raw_stmt = nullptr;
  int rc = sqlite3_prepare_v2(db_.get(), sql, -1, &raw_stmt, nullptr);
  UniqueStmt stmt(raw_stmt);
