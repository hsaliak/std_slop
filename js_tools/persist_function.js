return function(args) {
  try {
    ...
    if (args && Object.prototype.hasOwnProperty.call(args, "json_schema")) {
      if (typeof args.json_schema === "string") {
        ...
        try {
          ...
          return [false, "Invalid arguments: json_schema must be stringifiable if provided as object"];
        }
      }
    }

    if (typeof name !== "string" || name.trim() === "") {
      return [false, "Invalid arguments: name is required and must be a string"];
    }
    if (typeof code !== "string" || code.trim() === "") {
      return [false, "Invalid arguments: code is required and must be a string"];
    }
    ...
    if (tools[name] || globalThis[name]) {
      // Check if it's actually in the DB (meaning it's a re-definition of a persisted function)
      const collision_check_json = tools.query_db({sql: "SELECT 1 FROM js_functions WHERE name = ?", params: [name]});
      const collision_check = JSON.parse(collision_check_json);
      if (collision_check.length === 0) {
