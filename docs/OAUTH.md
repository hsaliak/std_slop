...
This document provides an overview of the authentication and project discovery architecture in `std::slop`.
...
*   **Interactive Authentication**: Handled by a standalone shell script (`slop_auth.sh`). This reduces the C++ application's complexity and avoids embedding interactive logic or transient dependencies within the binary.
*   **Token Management & Discovery**: Handled by the `OAuthHandler` class in C++. It manages token persistence, silent background refreshes using the `refresh_token`, and project discovery via internal Google Cloud Assist (GCA) endpoints.
...
## 4. Project Discovery
A key part of the flow is correctly identifying the Google Cloud Project ID. 
### Discovery Priority
