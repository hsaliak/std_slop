...
public:
    InjectionFinder(TSParser* inline_parser, ParsedMarkdown* parsed)
        : inline_parser_(inline_parser), parsed_(parsed) {}
...
private:
    void HandleInline(TSNode node) {
        ...
        TSTree* tree = ts_parser_parse_string(
            inline_parser_, nullptr, source.c_str() + start, end - start);
        
        if (tree) {
            parsed_->AddInjection({
                "markdown_inline", {start, end},
                std::shared_ptr<TSTree>(tree, ts_tree_delete)
            });
        }
    }
    ...
    void HandleCodeBlock(TSNode node) {
        ...
        if (!ts_node_is_null(content_node)) {
             parsed_->AddInjection({
                std::move(lang),
                {ts_node_start_byte(content_node), ts_node_end_byte(content_node)},
                nullptr 
            });
        }
    }
